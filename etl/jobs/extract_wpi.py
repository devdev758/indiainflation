"""Download WPI datasets from DPIIT and persist raw files."""
...
DPIIT_DATASETS: dict[str, str] = {
    "wpi_all_items": "https://eaindustry.nic.in/WPI_Release_Index/WPI_Data_2024.xlsx",
}
...
def extract_wpi(base_dir: Path | None = None) -> None:
    """Fetch WPI datasets and store them under raw directory."""

    base_dir = base_dir or Path(__file__).resolve().parents[1] / "data" / "raw" / "wpi"
    ensure_directory(base_dir)

    run_dir = base_dir / datetime.utcnow().strftime("%Y_%m")
    ensure_directory(run_dir)

    for name, url in DPIIT_DATASETS.items():
